/**
 * Generic renderer for kinematic fold models (gv-fold-model.js,
 * keel-fold-model.js, ...): each paper region is a convex polygon rendered
 * with a two-tone material (paper front / colored back, resolved by winding
 * so reflections flip color automatically) plus a dark outline. Rebuilding
 * the small mesh set every slider tick is cheap.
 */
...
export class FoldModelView {
  /**
   * @param compute  (stepId, t) => { faces, creases, flip } — see gv-fold-model.js
   * @param backColor  color of the paper's back side
   */
  constructor(compute, { backColor = 0x4488cc, elevation = -0.5 } = {}) {
    this.compute = compute;
    this.backColor = backColor;
    this.elevation = elevation; // camera height as a fraction of its distance
    this.container = null;
    this.stepId = 'paper';
    this.progress = 0;
    this.autoRotate = false;
    this.flipAngle = 0;
    this.stepFlips = false;
  }
  ...
  init(container) {
    ...
    // Fit the camera to the model's paper size (GV uses a 1.5-wide sheet,
    // the Keel unit a 3-wide strip).
    let span = 1.5;
    for (const f of this.compute(this.stepId, 0).faces) {
      for (const p of f.pts) span = Math.max(span, Math.abs(p[0]) , Math.abs(p[1]));
    }
    const dist = 1.6 * span;
    this.camera.position.set(0, this.elevation * dist, dist);
    ...
    // recentered on the working area (setStep computes the centering).
    ...
    this.backMat = new THREE.MeshStandardMaterial({
      color: this.backColor, side: THREE.BackSide, roughness: 0.85, metalness: 0,
      polygonOffset: true, polygonOffsetFactor: 1, polygonOffsetUnits: 1,
    });
  ...
  setStep(stepId, progress = 0) {
    ...
    const end = this.compute(stepId, 1);
    this.stepFlips = !!end.flip;
  ...
  _rebuild() {
    ...
    const { faces, creases } = this.compute(this.stepId, this.progress);
